...
if __name__ == "__main__":
    ...
    # 导入目标振动图像
    print('loading target pattern...')
    with open(os.path.join('../datasets/pattern_target', 'target_pattern.pkl'), 'rb') as f:
        data = pickle.load(f)
    ...
    # 打开文件以进行写入最优结果
    with open(f'save_SA_results/solution_{test_num}.txt', "w") as f:
        # 将数组转换为字符串并写入到文件的第一行
    ...
    # 保存energy轨迹
    with open(f'save_SA_results/path_energy_{test_num}.txt', 'wb') as f:
        for item in path_energy:
            f.write(("%s\n" % item).encode())

    # 将energy和对应solution都写入到pkl中
    # 保存到pkl文件
    with open(f'save_SA_results/path_energy+solution_{test_num}.pkl', 'wb') as f:
    ...
    plt.savefig(f'save_SA_results/result_compare_100_V{test_num}.png')
    # plt.show()
